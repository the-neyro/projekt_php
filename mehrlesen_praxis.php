...
if (isset($_POST["mehrlesen"]) && isset($_POST["aktiv_nr"]) && !isset($_POST["sub_aktiv_nr"])) {
...
if (isset($_POST["sub_aktiv_nr"])) {
    ...
    $aktiv_nr  = (int) $_POST["aktiv_nr"];
    $stmt = mysqli_prepare($con, "INSERT IGNORE INTO mehrlesen_umsetzung(aktiv_nr, uebung_id, user_nr) VALUES(?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "iii", $aktiv_nr, $uebung_id, $nutzer_nr);
    mysqli_stmt_execute($stmt);
}
?>

<h1>Mehr lesen – Praxis</h1>
...
<?php
$stmt = mysqli_prepare($con, "SELECT uebung_id, titel, beschreibung FROM mehrlesen_bd");
mysqli_stmt_execute($stmt);
...
while ($row = mysqli_fetch_assoc($erg)) {
    ...
    echo "<input type='hidden' name='aktiv_nr' value='3'>";
...
?>
    ...
    <p class="small">
      <a class="link" href="mehrlesen.php">← Zurück</a> ·
