<?php
$page_title = "Meditation – Deine Übungen";
include("header.php");
...
<h1>Meditation</h1>
<p class="small">Deine gespeicherten Meditationsübungen.</p>
...
$stmt = mysqli_prepare($con, "SELECT b.titel, b.beschreibung, mu.uebung_id
                               FROM meditation_bd b
                               JOIN meditation_umsetzung mu ON b.uebung_id = mu.uebung_id
                               WHERE mu.user_nr = ?");
...
if (mysqli_num_rows($erg) == 0) {
    echo "<p>Noch keine Meditationen gespeichert.</p>";
}
...
while ($row = mysqli_fetch_assoc($erg)) {
    ...
    echo "<a class='btn danger' href='meditation_delete_benutzer.php?id={$uebung_id}'>Löschen</a>";
    echo "</details>";
}
?>
